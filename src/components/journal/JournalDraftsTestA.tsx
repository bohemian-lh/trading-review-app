...
// ─── 组件 ──────────────────────────────────────────────────────────
export const JournalDraftsTestA: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-500">{drafts.length} 条A测试</div>
        ...
        <table className="border border-gray-200 rounded-lg overflow-hidden text-sm" style={{ tableLayout: 'fixed' }}>
          ...
          <tbody>
            {drafts.map(journal => {
              ...
              // 紧凑模式
              return (
                <tr key={journal.id} className="even:bg-gray-50/60 odd:bg-white transition-colors">
