...
const CHART_KEY = 'theory-system-total';
...
const lineConfigs: LineConfig[] = [
  { key: '系统', name: '系统总盈亏', color: '#0ea5e9' },
  { key: '理论-系统', name: '系统理论总盈亏', color: '#ef4444' },
];

const ALL_KEYS = lineConfigs.map(l => l.key);
const DEFAULT_KEYS = ['系统', '理论-系统'];

export const TheorySystemTotalChart: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i < maxLen; i++) {
      ...
      for (const l of lineConfigs) {
        ...
        point[l.key] = stats[i]?.profitSum ?? null;
      }
      data.push(point);
    }
    return data;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-900">图表2：系统总盈亏 vs 理论总盈亏</h3>
      ...
      <div ref={containerRef} className="select-none">
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={visibleData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <YAxis tick={{ fontSize: 12 }} tickCount={7} domain={yDomain} tickFormatter={(v: number) => v.toFixed(0)} label={{ value: '总盈亏', angle: -90, position: 'insideLeft' }} />
