...
describe('validateTradingRecord - entryType', () => {
  it('有效的 entryType p2前 通过', () => {
    const result = validateTradingRecord(makePartial({ entryType: 'p2前' as EntryType }));
    expect(result.errors.filter(e => e.field === 'entryType')).toHaveLength(0);
  });

  it('有效的 entryType p34 通过', () => {
    const result = validateTradingRecord(makePartial({ entryType: 'p34' as EntryType }));
    expect(result.errors.filter(e => e.field === 'entryType')).toHaveLength(0);
  });

  it('有效的 entryType p4后 通过', () => {
    const result = validateTradingRecord(makePartial({ entryType: 'p4后' as EntryType }));
    expect(result.errors.filter(e => e.field === 'entryType')).toHaveLength(0);
  });

  it('有效的 entryType 未知 通过', () => {
    const result = validateTradingRecord(makePartial({ entryType: '未知' as EntryType }));
    expect(result.errors.filter(e => e.field === 'entryType')).toHaveLength(0);
  });

  it('空的 entryType 报错', () => {
    const result = validateTradingRecord(makePartial({ entryType: '' as EntryType }));
  ...
  it('无效的 entryType 报错', () => {
    const result = validateTradingRecord(makePartial({ entryType: 'invalid' as EntryType }));
    const err = result.errors.find(e => e.field === 'entryType');
    expect(err).toBeDefined();
